...
//                    StringUtils.getInstance().getDefaultStorageLocation(mContext));
            ...
            StringUtils utils = new StringUtils(mContext);
            String folderPath = utils.GetPreferences(FOLDER_LOCATION);
...
//                                mContext.getString(R.string.created));
                        ...
                        if (reader.getNumberOfPages() == endPage - startPage + 1) {
                            StringUtils.getInstance().showSnackbar(mContext, "Entered split range will create Selected PDF again!");
                        } else {
                            document = new Document();
                            fileName = fileName.replace(pdfExtension,
...
//                                    mContext.getString(R.string.created));
                    ...
                    StringUtils.getInstance().showSnackbar(mContext, "Selected PDF can't be split because it has only 1 page!");
                }
            }
        ...
        try {
            ...
            switch (result) {
                case ERROR_PAGE_NUMBER:
                    StringUtils.getInstance().showSnackbar(mContext, "Invalid page number");
                    break;
                case ERROR_PAGE_RANGE:
                    StringUtils.getInstance().showSnackbar(mContext, "Invalid range input");
                    break;
                case ERROR_INVALID_INPUT:
                    StringUtils.getInstance().showSnackbar(mContext, "Invalid input");
                    break;
                default:
                    return true;
            }
