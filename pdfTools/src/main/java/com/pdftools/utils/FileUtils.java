...
import static com.pdftools.utils.Constants.AUTHORITY_APP;
import static com.pdftools.utils.Constants.PATH_SEPERATOR;
import static com.pdftools.utils.Constants.STORAGE_LOCATION;
import static com.pdftools.utils.Constants.pdfDirectory;
import static com.pdftools.utils.Constants.pdfExtension;
...
public class FileUtils {
    ...
    public void printFile(final File file) {
        ...
        String jobName = "PDFTools Document";
        if (printManager != null) {
            printManager.print(jobName, mPrintDocumentAdapter, null);
            new DatabaseHelper(mContext).insertRecord(file.getAbsolutePath(), "Printed");
        }
    }
    ...
    public void openFile(String path, FileType fileType) {
        if (path == null) {
            Snackbar.make(mContext.findViewById(android.R.id.content), R.string.error_path_not_found, Snackbar.LENGTH_LONG).show();
            return;
        }
    ...
    private void openFileInternal(String path, String dataType) {
        ...
        try {
            ...
            e.printStackTrace();
            Snackbar.make(mContext.findViewById(android.R.id.content), R.string.error_open_file, Snackbar.LENGTH_LONG).show();
        }
    }
     ...
     */
    public static String saveImage(Context context, String filename, Bitmap finalBitmap) {

        if (finalBitmap == null || checkIfBitmapIsWhite(finalBitmap))
            return null;

        StringUtils utils = new StringUtils(context);
        String root = utils.GetPreferences(FOLDER_LOCATION);
